...
class CMAdapter {
  ...
  getConfiguration() {
    ...
    } else if ('EditorOption' in EditorAPI) {
      // for monaco 0.19.3 onwards
      opts = EditorAPI.EditorOption
    }
  ...
  highlightRanges(ranges, className = 'findMatch') {
    ...
    this[decorationKey] = this.editor.deltaDecorations(
      ...
      ranges.map(range => ({
        ...
        options: {
          stickiness:
            EditorAPI.TrackedRangeStickiness.NeverGrowsWhenTypingAtEdges,
          zIndex: 13,
          className,
